...
class DinoPlayer:
    ...
    def play(self, source: Optional[str] = None):
        ...
        self.duration = 0.0
        self._publish_state()
        threading.Thread(target=self._monitor_mpv, args=(play_gen,), daemon=True).start()
        threading.Thread(target=self._poll_status, args=(play_gen,), daemon=True).start()

    def _monitor_mpv(self, play_gen: int):
    ...
    def _poll_status(self, play_gen: int):
        while self._running and play_gen == self._play_gen and self.state == "playing":
            time.sleep(STATUS_SECONDS)
            if play_gen != self._play_gen or self.state != "playing":
                return
